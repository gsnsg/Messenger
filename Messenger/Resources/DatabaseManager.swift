...
extension DatabaseManager {
        ...
        ref.observeSingleEvent(of: .value) { [weak self] (snapshot) in
            ...
            self?.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { (snapshot) in
                if var conversations = snapshot.value as? [[String: Any]] {
                    ...
                    // create
                    self?.database.child("\(otherUserEmail)/conversations/").setValue([recepientConversationData])
                }
            }
            // Update current user
        ...
        database.child("\(id)/messages").observe(.value) { (snapshot) in
            ...
            let messages: [Message] = value.compactMap { (dictionary)  in
                guard let name = dictionary["name"] as? String,
                      ...
                      let messagesId = dictionary["id"] as? String,
    ...
    public func sendMessage(to conversation: String, otherUserEmail: String, name: String, newMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        self.database.child("\(conversation)/messages").observeSingleEvent(of: .value) { [weak self] (snapshot) in
            ...
            var messageDate = newMessage.sentDate
            let dateString = ChatViewController.dateFormatter.string(from: messageDate)
            var message = ""
